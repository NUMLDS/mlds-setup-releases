...
`mlds-setup` is a small program you run **once per laptop**. It checks that
Git and the GitHub CLI are installed, signs you in to GitHub, makes sure you
have an SSH key and that it is on your GitHub account, and then registers you
with the MLDS servers so you can `ssh` in. It takes about two minutes.

You can run it again any time. If you ever can't log in, running it again is
the fix.

## Before you start

1. **A GitHub account**, with the username you gave us on the intake survey.
   If you changed your username since, tell the sysadmin (Mateusz).
...
enabled for you but refused the key; tell the sysadmin (Mateusz) which one.
...
| `GitHub account '…' is not on file for anyone` | tell the sysadmin (Mateusz) your exact GitHub username, then re-run |
...
| `report saved to ~/mlds-setup-report.json` | the MLDS server couldn't be reached; get on campus or the VPN and re-run, or send that file to the sysadmin (Mateusz) |
| a host in the login test says `permission denied` or `timed out during login` | that server isn't set up for your account yet; tell the sysadmin (Mateusz) which one. Your key is fine |
| `host key changed` | don't log in to that host; tell the sysadmin (Mateusz) |
| `Permission denied` when you `ssh` later | run `mlds-setup` again. Still stuck? Send the sysadmin (Mateusz) the output |
...
| Posit or psql rejects the password you set | give it a few seconds and retry; still no? tell the sysadmin (Mateusz) which server |
